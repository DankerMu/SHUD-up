...
def plot_aspect_delta_timeseries(
    *,
    ...
    day_b, rn_b_s, rn_b_n, rn_b_o = group_means(out_base, f"{basin}.rn_t.dat")
    day_t, rn_t_s, rn_t_n, rn_t_o = group_means(out_tsr, f"{basin}.rn_t.dat")
    _, et_b_s, et_b_n, et_b_o = group_means(out_base, f"{basin}.eleveta.dat")
    _, et_t_s, et_t_n, et_t_o = group_means(out_tsr, f"{basin}.eleveta.dat")

    if day_b.shape != day_t.shape or not np.allclose(day_b, day_t, rtol=0.0, atol=0.0):
        raise PlotError(f"Baseline and TSR time axes differ for {basin}; cannot plot aspect TSR effect.")
...
def main() -> int:
    ...
    plot_aspect_delta_timeseries(
        ...
        title="heihe: aspect-group TSR effect (TSR−BASE)",
    )
    ...
    plot_aspect_delta_timeseries(
        ...
        title="qhh: aspect-group TSR effect (TSR−BASE)",
    )
    plot_aspect_storage_delta_timeseries(
        basin="qhh",
        out_base=args.qhh_base,
